...
max_pmcid = get_id_from_doi(max_pubmed_DOI,output_type="pmcid")
"""

max_pmcid = "PMC7158585"
max_pubmed_DOI = "10.1016/S0140-6736(20)30607-3"
max_pmid = get_id_from_doi(max_pubmed_DOI,output_type="pmid")
#max_citation_number = max_citation_row['Citation Number']
...
pdf_urls, abstract_list, relatedpapers = extract_pdf_similarity(similarity_output_path,type="pubmed")
#将核心论文加入pdf_urls
pdf_urls.append(max_url)
print("The last PDF URLs found:")
print(pdf_urls)
...
reports={}
text=""
initial_response = generate_report(text, question, prompt_type="idea", paper=core_paper,
                                   relatedPaper=relatedpapers)
reports["idea"] = initial_response
problem,rationale = extract_content(initial_response, "Problem:")


second_response = generate_report(text, question, prompt_type="method", paper=core_paper,
                                  relatedPaper=relatedpapers,
                                  researchProblem=problem, researchProblemRationale=rationale)
reports["method"] = second_response
method,method_rationale = extract_content(second_response, "Method:")


final_response = generate_report(text, question, prompt_type="experiment", paper=core_paper,
                                 relatedPaper=relatedpapers,
                                 researchProblem=problem, researchProblemRationale=rationale,
                                 scientificMethod=method, scientificMethodRationale=method_rationale)
reports["experiment"] = final_response
...
for type in types:
    text = context[type]
    reports[type] = generate_report(text, question, paper=core_paper,relatedPaper=relatedpapers,prompt_type=type)
